  ...
  <div class="hotComments">
    <div class="comments" v-if="commentType != '' && commentType != 'music' && isHotComment">
      ...
      <div class="submitCommentButton">
        <el-button size="mini" round @click="commentMode ? submitComment() : submitFloorComment()" class="submitComment">评论</el-button>
      </div>
    </div>
    <!-- 音乐单曲评论 -->
    <div
  ...
  methods: {
    submitComment() {
      if(!this.$store.state.isLogin) {
        this.$message.warning("之后登录后才能评论")
        return null
      }
      ...
      getComment({
        ...
        if(res.data.code === 200) {
          if(this.commentType === "music") this.isCommentDialogShow = false;
          this.commentInput = ""
          this.$emit("getComment");
        }else{
          this.$message.error("评论失败，请稍候重试")
        }
      }).catch(err=>{
    ...
    likeCurrentComment(flag, cid) {
      ...
      if(!this.$store.state.isLogin) {
        this.$message.warning("只有登陆后才能点赞！")
        return null;
      }
    ...
    floorComment(commentId, nickName) {
      ...
      if(this.isHotComment) this.$emit("transToHotComment", {commentId, nickName})
